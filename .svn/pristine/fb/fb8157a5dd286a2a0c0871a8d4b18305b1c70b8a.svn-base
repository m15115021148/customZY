package com.bluemobi.zhongyou.view.scan;
...
public class QRCoverView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (scannerW == 0f || scannerH == 0f) {
            viewFinderRect = new RectF(left, top, dip2px(context, 180), top + dip2px(context, 180));
        } else {
    ...
    public QRCoverView setCoverViewCorner(int cornerH, int cornerW, boolean isDp) {
        if (isDp) {
            this.cornerLength = dip2px(context, cornerH);
            this.cornerWidth = dip2px(context, cornerW);
        } else {
    ...
    public QRCoverView setCoverViewScanner(int width, int height) {
        //配置扫描框的宽高
        this.scannerH = dip2px(context, height);
        this.scannerW = dip2px(context, width);
        //重新获取扫描框的左边距和上边距
